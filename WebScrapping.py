...
# -------------------------------------------------------------------
# SERVIDOR HTTP DE SALUD PARA RENDER
# -------------------------------------------------------------------
...
def run_health_check_server():
    ...
    print(f"[+] Servidor HTTP de salud escuchando en el puerto {port}", flush=True)
    server.serve_forever()

# -------------------------------------------------------------------
# FUNCIONES AUXILIARES Y SCRAPING
# -------------------------------------------------------------------
...
def check_and_log_deal(title: str, price: str, url: str, source: str):
    ...
    origenes = ["buenos aires", "ezeiza", "aeroparque", "cordoba", "córdoba", "mendoza", "argentina", "rosario", "desde", "from", "flights from", "departing", "non-stop", "roundtrip", "round-trip"]
    destinos = ["bariloche", "salta", "iguazu", "iguazú", "ushuaia", "calafate", "tucuman", "tucumán", "jujuy", "mendoza", "cordoba", "córdoba", "neuquen", "neuquén", "catamarca", "san juan", "salta", "cabotaje", "nacionales", "europa", "europe", "asia", "madrid", "barcelona", "roma", "milan", "milán", "paris", "parís", "londres", "tokio", "tokyo", "bangkok", "estambul", "istanbul", "rio", "río", "miami"]
        ...
        try:
            ...
            for sitio in sitios:
                ...
                try:
                    ...
                    cards = await page.query_selector_all("article, .post, .card, .entry-preview, .blog-post, .post-item, .entry, h2.entry-title, h3.entry-title, h2 a, h3 a")
                    cards = cards[:10]
                    encontrados = 0
                    urls_procesadas = set()
...
async def iniciar_telegram():
    if not API_ID or not API_HASH:
        print("[!] No se definieron API_ID o API_HASH. Omitiendo Telethon.")
        return
    ...
    async def handler(event):
        ...
        if is_relevant_deal(texto):
            send_telegram_alert(title=texto, url="", source="Canal Telegram")
...
if __name__ == "__main__":
    # 1. Iniciar servidor de salud en un hilo independiente (abrir puerto de Render)
    threading.Thread(target=run_health_check_server, daemon=True).start()
    
    # 2. Iniciar el bucle principal de asyncio
    asyncio.run(main())
